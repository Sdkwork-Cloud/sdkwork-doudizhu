...
const FAMILY_DEFAULTS = {
  'sdkwork-doudizhu-app-sdk': { surface: 'app', clientName: 'SdkworkDoudizhuAppClient' },
  'sdkwork-doudizhu-backend-sdk': { surface: 'backend', clientName: 'SdkworkDoudizhuBackendClient' },
};
...
function createGenerationPlans(rootDir, options) {
  ...
  assert.equal(assembly.sdkOwner, 'sdkwork-doudizhu');
  ...
  for (const language of requestedLanguages) {
    if (!SUPPORTED_LANGUAGES.has(language)) {
      throw new Error(`Unsupported Doudizhu standard SDK language: ${language}.`);
    }
  }
  ...
  const defaults =
    FAMILY_DEFAULTS[options.sdkFamily] ?? FAMILY_DEFAULTS['sdkwork-doudizhu-app-sdk'];
...
export function generateDoudizhuSdk(options = {}, rootDir = process.cwd()) {
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  try {
    const result = generateDoudizhuSdk(parseArgs(process.argv.slice(2)));
