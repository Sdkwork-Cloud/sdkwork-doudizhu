...
const pcRoot = path.join(root, 'apps/sdkwork-doudizhu-pc');
const packagesDir = path.join(pcRoot, 'packages');
const applicationCode = 'doudizhu';
...
test('PC app root uses sdkwork-doudizhu-pc directory and manifest', () => {
  ...
  assert.equal(manifest.app.key, 'sdkwork-doudizhu-pc');
  assert.equal(manifest.runtime.family, 'pc');
});

test('PC packages follow sdkwork-doudizhu-pc-* naming', () => {
  const packageDirs = fs
    .readdirSync(packagesDir, { withFileTypes: true })
    .filter((entry) => entry.isDirectory())
    .map((entry) => entry.name);

  assert.ok(packageDirs.length > 0, 'expected PC packages under apps/sdkwork-doudizhu-pc/packages');
