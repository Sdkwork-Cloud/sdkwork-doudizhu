...
test('route manifests declare WebRequestContext on protected routes', () => {
  const manifestPath = path.join(
    root,
    'sdks/_route-manifests/app-api/sdkwork-routes-doudizhu-app-api.route-manifest.json',
  );
  const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
  const protectedRoute = manifest.routes.find((route) => route.path.includes('/doudizhu/matches'));
