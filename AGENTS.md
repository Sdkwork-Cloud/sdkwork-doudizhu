...
Root manifest: `sdkwork.app.config.json` (application code `doudizhu`, domain `game`).

Per-surface manifests live under `apps/sdkwork-doudizhu-pc/sdkwork.app.config.json` and sibling app roots when added.
...
- `sdkwork.app.config.json`: runtime/SDK wiring for the doudizhu application.
