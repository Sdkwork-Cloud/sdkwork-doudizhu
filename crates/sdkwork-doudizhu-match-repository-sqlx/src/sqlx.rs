...
use sdkwork_doudizhu_match_service::{
...
async fn list_postgres(
    ...
    let rows = if let Some(status) = status {
        sqlx::query_as::<_, MatchRow>(
            "SELECT id, match_code, title, summary, mode, status FROM ddz_match \
    ...
    let total: i64 = if let Some(status) = status {
        sqlx::query_scalar(
            "SELECT COUNT(*) FROM ddz_match WHERE tenant_id = $1 AND deleted_at IS NULL AND status = $2",
        )
        .bind(tenant_id)
        .bind(status)
        .fetch_one(pool)
        ...
        sqlx::query_scalar(
            "SELECT COUNT(*) FROM ddz_match WHERE tenant_id = $1 AND deleted_at IS NULL",
        )
        .bind(tenant_id)
        .fetch_one(pool)
        .await
...
async fn list_sqlite(
    ...
    let total: i64 = if let Some(status) = status {
        sqlx::query_scalar(
            "SELECT COUNT(*) FROM ddz_match WHERE tenant_id = ? AND deleted_at IS NULL AND status = ?",
        )
        .bind(tenant_id)
        .bind(status)
        .fetch_one(pool)
        ...
        sqlx::query_scalar(
            "SELECT COUNT(*) FROM ddz_match WHERE tenant_id = ? AND deleted_at IS NULL",
        )
        .bind(tenant_id)
        .fetch_one(pool)
        .await
