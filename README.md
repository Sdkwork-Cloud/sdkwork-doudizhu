...
SDKWork doudizhu (斗地主) application root. Provides match, room, and leaderboard capabilities aligned with `../sdkwork-specs`.
...
| `apps/sdkwork-doudizhu-pc/` | PC browser/desktop React application root |
| `crates/` | Rust services, repositories, API server |
| `database/` | `sdkwork-database` lifecycle assets (`moduleId=doudizhu`, prefix `ddz_`) |
...
- **HTTP**: `sdkwork-web-framework` via `crates/sdkwork-api-doudizhu-standalone-gateway`
